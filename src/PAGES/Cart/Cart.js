...
const Cart = () => {
  ...
  // let cartItems = [];
  // cart.forEach(item => {
  //   cartItems.push([...cartItems,{product_id:item.productdata.ProductId,quantity:item.quantity+1}])
  // })
  // console.log(cartItems);
  const uploadCartData = async () => {
    let cartdata = JSON.parse(localStorage.getItem("cart"));
    const storedCartData = cartdata;
    let cartwithIdQuantity = {};
    storedCartData.forEach((item) => {
      let id = String(item.product_id);
      cartwithIdQuantity[id] = item.quantity;
    });
    console.log(cartwithIdQuantity);
    try {
      const response = await fetch("/api/cart", {
        ...
        body: JSON.stringify({ cart: cartwithIdQuantity }),
      });
      ...
      console.error("Error upload cart:", error);
    }
  };
